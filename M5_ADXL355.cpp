...
ADXL355::ADXL355 (uint8_t sckPin,uint8_t miPin,uint8_t moPin,uint8_t csPin, uint32_t frequency) : spiSettings (frequency, MSBFIRST, SPI_MODE0), sckPin (sckPin), miPin (miPin), moPin (moPin), csPin (csPin) {}

//==============================================================================

void ADXL355::begin() {
  //SPI.begin(SCK, MISO, MOSI, SS);
  //spi2.begin(26, 36, 32, csPin);
  //spi2.begin(13, 14, 32, csPin);
  //spi2.begin(19, 27, 32, csPin);
...
void ADXL355::read(uint8_t address, void* dest, size_t numberOfRegisters) {
  ...
  spi2.transfer((address << 1) | 1);
  spi2.transfer(dest, numberOfRegisters);
  digitalWrite(csPin, HIGH);
  spi2.endTransaction();
}
...
void ADXL355::write(uint8_t address, void* src, size_t numberOfRegisters) {
  ...
  spi2.beginTransaction(spiSettings);
  digitalWrite(csPin, LOW);
  spi2.transfer(address << 1);
  spi2.transfer(src, numberOfRegisters);
  digitalWrite(csPin, HIGH);
  spi2.endTransaction(); 
}
